...
export default class CopMemoryGenerator{
    ...
    constructor(configPath: string){
        const file= fs.readFileSync(configPath).toString()//("../assembler/src/VSHAssemblerNaive/resources/VirtualShockIsa.js").toString();
        this.isa= JSON.parse(file).isa
    ...
    getCellByAddress(address:number,copMemory:CopMemoryCell[]):CopMemoryCell{
        for(const cell of copMemory){
            ...
            if(!this.isa.hasOwnProperty(copName)) throw new Error("missing instruction '"+copName+"' in ISA map")
            const copBits= this.isa[copName].copBytes;
